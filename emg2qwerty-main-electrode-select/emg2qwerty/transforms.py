...
class ElectrodeSelector:
    ...
    Input must be of shape (T, bands, C, ...) where dim ``channel_dim``
    corresponds to the electrode channel axis.  Returns a tensor with that
    axis reduced to ``len(indices)`` entries.

    When ``per_band_indices`` is set, each band can use a different set of
    channel indices (all sub-lists must have the same length).  This takes
    precedence over ``indices``.

    When both are ``None`` the transform is a no-op (all channels kept).

    Args:
        indices: Electrode channel indices to keep (same for every band).
            ``None`` keeps all.
        per_band_indices: A list of index lists, one per band, allowing
            different channels per band.  ``None`` falls back to ``indices``.
        band_dim: The band dimension in the input tensor. (default: 1)
        channel_dim: The electrode channel dimension. (default: 2)
    """

    indices: Sequence[int] | None = None
    per_band_indices: Sequence[Sequence[int]] | None = None
    band_dim: int = 1
    channel_dim: int = 2

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        if self.per_band_indices is not None:
            bands = tensor.unbind(self.band_dim)
            assert len(bands) == len(self.per_band_indices), (
                f"Expected {len(self.per_band_indices)} bands, got {len(bands)}"
            )
            ch_dim = self.channel_dim - (1 if self.band_dim < self.channel_dim else 0)
            selected = []
            for band_tensor, band_idx in zip(bands, self.per_band_indices):
                idx = torch.tensor(band_idx, dtype=torch.long, device=tensor.device)
                selected.append(band_tensor.index_select(ch_dim, idx))
            return torch.stack(selected, dim=self.band_dim)

        if self.indices is None:
            return tensor
        idx = torch.tensor(self.indices, dtype=torch.long, device=tensor.device)
        return tensor.index_select(self.channel_dim, idx)
