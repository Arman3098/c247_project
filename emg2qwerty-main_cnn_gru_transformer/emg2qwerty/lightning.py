...
class CNNGRUTransformerCTCModule(pl.LightningModule):
    NUM_BANDS: ClassVar[int] = 2
    ELECTRODE_CHANNELS: ClassVar[int] = 16
    ...
    def __init__(
        ...
        gru_output_size = gru_hidden_size * 2
        self.gru_proj = (
            nn.Linear(gru_output_size, num_features)
            if gru_output_size != num_features
            else nn.Identity()
        )
        self.gru_norm = nn.LayerNorm(num_features)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        x = self.gru_proj(gru_out)
        x = self.gru_dropout(x) + residual
        x = self.gru_norm(x)
